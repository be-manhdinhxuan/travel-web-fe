...
// ===== ISSUES DATA =====
var DEFAULT_ISSUES = [
  {id:'ISS-9921', customer:'Lê Văn An',   booking:'BK-9921', type:'urgent',    status:'urgent',     title:'Hỗ trợ khẩn cấp: Mất hộ chiếu tại sân bay', desc:'Đoàn đang ở Cửa quốc tế Nội Bài', time:'10:45 AM, Hôm nay'},
  {id:'ISS-9920', customer:'Maria Sharapova', booking:'BK-9920', type:'issue', status:'processing', title:'Sự cố: Phòng khách sạn không đúng mô tả',      desc:'Phòng nhỏ hơn, thiếu tiện nghi hạng đồng', time:'08:30 AM, Hôm nay'},
  {id:'ISS-9918', customer:'Trần Minh Hoàng', booking:'BK-9918', type:'praise', status:'done',      title:'Phản hồi: Khen ngợi hướng dẫn viên',          desc:'Khách hàng rất hài lòng với HDV Nam',        time:'Hôm qua'},
  {id:'ISS-9915', customer:'Nguyễn Thị Bình', booking:'BK-9915', type:'issue', status:'pending',   title:'Sự cố: Hủy tour do lý do sức khỏe',           desc:'Yêu cầu hoàn phí theo chính sách bảo hiểm',  time:'2 ngày trước'},
];

function nvGetIssues() {
  try { var r = localStorage.getItem(NV_ISSUES_KEY); return r ? JSON.parse(r) : DEFAULT_ISSUES; } catch(e) { return DEFAULT_ISSUES; }
}
function nvSaveIssues(issues) { localStorage.setItem(NV_ISSUES_KEY, JSON.stringify(issues)); }
...
function nvRenderBookings() {
  ...
  tbody.innerHTML = all.map(function(item) {
    ...
    return '<tr>' +
      ...
      '<td><span class="nv-badge ' + (b.status==='done'?'nv-badge-done':'nv-badge-processing') + '">' + (b.status==='done'?'ĐÃ HOÀN THÀNH':'ĐANG DIỄN RA') + '</span></td>' +
    '</tr>';
  }).join('');
}
...
function nvLogout() {
  ...
  else { sessionStorage.removeItem('vt_user'); window.location.href = '1dangnhap-nhanvien.html'; }
}