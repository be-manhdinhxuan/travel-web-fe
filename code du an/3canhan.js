...
function cpBuildBookingRow(b) {
  ...
  return '<div class="cp-booking-item">'+
    ...
    '<div class="cp-booking-right">'+
      ...
      '<button class="cp-detail-btn" onclick="cpShowDetail(\''+escQ(tour)+'\',\''+escQ(date)+'\',\''+escQ(guests)+'\',\''+escQ(pay)+'\',\''+escQ(total)+'\',\''+escQ(code)+'\')">Xem chi tiết</button>'+
    '</div>'+
  '</div>';
}
...
function cpShowDetail(tour,date,guests,pay,total,code,id) {
  ...
  set('cpMdTour',tour); set('cpMdDate',date); set('cpMdGuests',guests);
  set('cpMdPayment',pay); set('cpMdTotal',total); set('cpMdCode',code);
  document.getElementById('cpModalOverlay').style.display='block';
